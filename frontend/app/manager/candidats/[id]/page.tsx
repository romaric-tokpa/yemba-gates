...
import { ArrowLeft, Upload, FileText, Tag, Mail, Phone, Calendar, User, Briefcase, MapPin, Clock, Eye, Edit, X, Plus, Save } from 'lucide-react'
...
export default function ManagerCandidateDetailPage() {
  ...
  const loadCandidate = async () => {
    try {
      ...
      // Log pour débogage
      console.log('📋 [MANAGER] Données candidat reçues:', {
        id: candidateData.id,
        profile_title: candidateData.profile_title,
      ...
      {!isEditing && (
        <div className="mt-6">
          <div className="bg-white rounded-lg shadow p-6">
            <button
              onClick={() => setIsEditing(true)}
              className="w-full sm:w-auto flex items-center justify-center px-6 py-3 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition-colors font-medium"
            >
